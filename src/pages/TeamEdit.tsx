...
export default function TeamEdit() {
  ...
  const { updateTeam, getTeamById, teams } = useTeams();
  ...
  useEffect(() => {
    if (team) {
      ...
      setSelectedManagers(team.managerIds);
  ...
  const validate = () => {
    ...
    } else if (teams.some(t => t.id !== id && t.name.toLowerCase() === name.toLowerCase())) {
  ...
  return (
    <main className="flex-1 p-6">
      <Button variant="ghost" onClick={() => navigate(`/teams/${id}`)} className="mb-4">
